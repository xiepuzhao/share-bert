...
def train(args, train_dataset, model, tokenizer, entity_set, mention_set, config):
    ...
    for _ in train_iterator:  # 遍历epoch
        ...
        for step, batch in enumerate(epoch_iterator):  # 遍历batch
            ...
            # todo:能不能ranking bert直接用 recall bert的输出，不经过gpu-cpu-gpu的转换，以及for的重新生成example
                ...
                for num1, candidate in enumerate(candidate_set[num]):
                    ...
                    if num1 == 0:
                        examples.append(LinkingExample(guid='data', text_a=mention_text, text_b=entity_text,
                                                       label='1', mention_id=mention_id[num]))
                    else:
                        examples.append(LinkingExample(guid='data', text_a=mention_text, text_b=entity_text,
                                                       label='0', mention_id=mention_id[num]))
            ...
            if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                ...
                recall_loss_scalar = (tr_recall_loss - logging_recall_loss) / args.logging_steps
                rank_loss_scalar = (tr_rank_loss - logging_rank_loss) / args.logging_steps
    ...
    return global_step, (tr_rank_loss+tr_recall_loss) / global_step
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            assert logits.shape[1] == len(entity_set)
            logits = torch.abs(logits)
            _, index = torch.sort(logits, dim=1, descending=True)
            index = index[:, :64]
            for id in range(index.shape[0]):
            ...
            mention_id = mention_id.detach().cpu().numpy()
            mention_id = mention_id.tolist()
            entity_label = batch[3]
            entity_label = entity_label.detach().cpu().numpy()
            entity_label = entity_label.tolist()
            for num in range(len(candidate_set)):
                for num1, candidate in enumerate(candidate_set[num]):
                    if candidate == entity_label[num]:
                        recall_num += 1
                    entity_text = entity_set[candidate]
                    mention_text = mention_set[mention_id[num]]
                    if num1 == 0:
    ...
    results["rank_arruracy"] = rightnum/total_num
