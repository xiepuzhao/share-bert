...
def train(args, train_dataset, model, tokenizer, entity_set, mention_set, config):
    ...
    # with open(r'./named_parameters.txt', 'w') as f:
    #     f.write("named_parameters")
    #     f.write('\n')
    ...
    for _ in train_iterator:  # 遍历epoch
        ...
        # todo:放到一个function里面
        RankDataset = load_rank_examples(args, entity_set, epoch_iterator, mention_set,
                                                                         model, steps_trained_in_current_epoch,
                                                                         tokenizer)
        global_rank_step, tr_rank_loss = training_rank(RankDataset, args, model, optimizer, scheduler, tb_writer,
                                                     tokenizer)
        #             for key, value in logs.items():
...
def training_rank(RankDataset, args, model, optimizer, scheduler, tb_writer, tokenizer):
        ...
        if (rank_step + 1) % args.gradient_accumulation_rank_steps == 0:
            ...
            if args.local_rank in [-1, 0] and args.logging_rank_steps > 0 and global_rank_step % args.logging_rank_steps == 0:
                ...
                if (
                    ...
                    # todo:每次调用时保存模型，比较前后几次模型参数是否发生改变
            ...
            if args.local_rank in [-1, 0] and args.save_steps > 0 and global_rank_step % args.save_steps == 0:
                # Save model checkpoint
                output_dir = os.path.join(args.output_dir, "checkpoint-{}".format(global_rank_step))
    ...
    return global_rank_step, tr_rank_loss


def load_rank_examples(args, entity_set, epoch_iterator, mention_set, model, steps_trained_in_current_epoch, tokenizer):
    preds = None
    labels = None
    mention_ids = None
    ...
    # todo:放到一个function里面
    # todo:专门写一个datalodar来加载数据，进行batch处理
    features = []
    print("total num of examples{}".format(len(examples)))
    recurrent_step = len(examples) // 10000
    pool = multiprocessing.Pool(processes=20)  # 创建4个进程
    result = []
    for i in range(recurrent_step):
        result.append(pool.apply_async(convert_rank_examples_to_features, (
        examples[10000 * i:10000 * (i + 1)], tokenizer, args.max_seq_length, "rank", None, "classification", False,
        tokenizer.convert_tokens_to_ids([tokenizer.pad_token])[0], 0, True, 10000 * (i + 1)
        )))
...
def load_and_cache_examples(args, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if evaluate is True:
            features = torch.load(cached_features_file)[:100]
        else:
            features = torch.load(cached_features_file)[:5000]
    else:
...
def main():
    ...
    model_class = EntityRecallRank(config=config)
    model = model_class.from_pretrained(
        args.model_name_or_path,  # 从model_name_or_path加载模型
        from_tf=bool(".ckpt" in args.model_name_or_path),
        config=config,
        cache_dir=args.cache_dir if args.cache_dir else None,
        state_dict=model_class.state_dict(),
    )
