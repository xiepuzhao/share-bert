    ...
    for _ in train_iterator:  # 遍历epoch
        ...
        for step, batch in enumerate(epoch_iterator):  # 遍历batch
            ...
            for id in range(index.shape[0]):
                if torch.sum(torch.eq(index[id, :], batch[3][id].item())).item() == 0:
                    index[id, 0] = batch[3][id].item()
                else:
                    id_list = torch.eq(index[id, :], batch[3][id].item()).detach().cpu().numpy()
            ...
            mention_id = batch[4]
            mention_id = mention_id.detach().cpu().numpy()
            mention_id = mention_id.tolist()
            logger.info("candidate_set shape{},{}".format(len(candidate_set), len(candidate_set[0])))
            logger.info("mention_id length".format(len(mention_id)))
            # print("mention_id shape"+str(len(mention_id)))
            # print("candidate_set")
            # print(candidate_set)
            ...
            #     if rank_pooled_output.shape[0] == 0:
            #         rank_pooled_output = pooled_outputs
            #     else:
            #         rank_pooled_output = torch.cat((rank_pooled_output, pooled_outputs), dim=0)
            # dropout = nn.Dropout(config.hidden_dropout_prob)
            # classifier = nn.Linear(config.hidden_size, 1)
            # rank_pooled_output = dropout(rank_pooled_output)
            # RankLogits = classifier(rank_pooled_output)  # shape(batch_size, num_label)
            # loss_fct = CrossEntropyLoss()
            # # rank_label无需计算梯度
            # rank_label = torch.zeros(len(candidate_set), dtype=torch.long)
            # rankloss = loss_fct(RankLogits.view(-1, 64), rank_label.view(-1))

            # if args.n_gpu > 1:
            #     rankloss = rankloss.mean()  # mean() to average on multi-gpu parallel training
            # if args.gradient_accumulation_steps > 1:
            #     rankloss = rankloss / args.gradient_accumulation_rank_steps

            # if args.fp16:
            #     with amp.scale_loss(rankloss, optimizer) as scaled_loss:
            #         scaled_loss.backward()
            # else:
            #     rankloss.backward()
            # if (rank_step + 1) % args.gradient_accumulation_rank_steps == 0:
        ...
        with torch.no_grad():
            ...
            for rank_step, rank_batch in enumerate(rank_epoch_iterator):
                ...
                with torch.no_grad():
                    ...
                    eval_loss += rank_eval_loss.mean().item()
