    ...
    for _ in train_iterator:  # 遍历epoch
        ...
        for step, batch in enumerate(epoch_iterator):  # 遍历batch
            ...
            if (step + 1) % args.gradient_accumulation_recall_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_recall_step % args.logging_steps == 0:
                    logs = {}
                    if (
        ...
        for rank_step, rank_batch in enumerate(rank_epoch_iterator):
            ...
            if (rank_step + 1) % args.gradient_accumulation_rank_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_rank_step % args.logging_steps == 0:
                    logs = {}
                    if (
                            args.local_rank == -1 and args.evaluate_during_training
                    ):  # Only evaluate when single GPU otherwise metrics may not average well
    ...
    if recall is False:
        ...
        for rank_step, rank_batch in enumerate(rank_epoch_iterator):
            ...
            with torch.no_grad():
                ...
                eval_rank_loss += rank_eval_loss.mean().item()
